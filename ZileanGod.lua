...
function OnLoad()
    ...
    Menu:addSubMenu(myHero.charName.." - Combo Settings", "Combo")
        ...
        Menu.Combo:addParam("useE", "Use E on enemy in combo", SCRIPT_PARAM_ONOFF, true)
        Menu.Combo:addParam("Eself", "Use E on yourself if out of range", SCRIPT_PARAM_ONOFF, true)
        Menu.Combo:addParam("panicult", "Smart Ultimate", SCRIPT_PARAM_ONOFF, true)
        Menu.Combo:addParam("hp", "What % to ult", SCRIPT_PARAM_SLICE, 20, 0, 100, 0)
...
function Combo()
    ...
    if ValidTarget(target) then
        ...
        if Menu.Combo.Eself then
            if E:IsReady() and GetDistance(target) < 1150 and GetDistance(target) > 700 then
                CastSpell(_E, myHero)
            end
        end
...
function Harass()
    ...
    if myHero.mana / myHero.maxMana * 100 >= Menu.Harass.Mana then
        if ValidTarget(target) then
            ...
            if Menu.Harass.useW and not Q:IsReady() then
                W:Cast()
            end
        end
    end
