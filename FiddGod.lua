...
function CheckUpdate()
    if AUTOUPDATES then
        ...
        ToUpdate.VersionPath = "raw.githubusercontent.com/Artrilo/Bot-Of-Legends/master/version/FiddGod.version"
        ToUpdate.ScriptPath = "raw.githubusercontent.com/Artrilo/Bot-Of-Legends/master/FiddGod.lua"
...
function OnLoad()
    ...
    Q = _Spell({Slot = _Q, DamageName = "Q", Range = 525, Type = SPELL_TYPE.TARGETTED}):AddDraw()
    W = _Spell({Slot = _W, DamageName = "W", Range = 650, Type = SPELL_TYPE.TARGETTED}):AddDraw()
    ...
    R = _Spell({Slot = _R, DamageName = "R", Range = 800, Width = 200, Delay = 0.25, Speed = math.huge, Aoe = true, Collision = false, Type = SPELL_TYPE.CIRCULAR}):AddDraw()
    ...
    Menu:addSubMenu(myHero.charName.." - Combo Settings", "Combo")
        ...
        Menu.Combo:addSubMenu("E Settings", "E")
            Menu.Combo.E:addParam("useE", "Use E", SCRIPT_PARAM_ONOFF, true)
    ...
    Menu:addSubMenu(myHero.charName.." - Auto Settings", "Auto")
        ...
        Menu.Auto:addSubMenu("Use E To Interrupt Gapclosers", "E")
        _Interrupter(Menu.Auto.E):CheckGapcloserSpells():AddCallback(function(target) E:Cast(target) end)
        Menu.Auto:addSubMenu("Use E To Interrupt Channeling Spells", "E")
        _Interrupter(Menu.Auto.E):CheckChannelingSpells():AddCallback(function(target) E:Cast(target) end)
...
function Combo()
    ...
    if ValidTarget(target) then
        ...
        if Menu.Combo.Q.useQ and not W:IsReady() then
            Q:Cast(target)
        end
        if Menu.Combo.E.useE and not W:IsReady() then
            E:Cast(target)
        end
...
function Buff()
    if TargetHaveBuff("fiddlebuff", target) then
...
function OnCreateObj(obj)
    if obj and obj.name then
        if obj.name:lower():find("Fiddlestick_Base_Drain.troy")  then
            Drain = true
        end
    end
end
