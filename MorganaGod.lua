...
local ScriptName = "MorganaBanana"
...
function CheckUpdate()
    if AUTOUPDATES then
        ...
        ToUpdate.VersionPath = "raw.githubusercontent.com/Artrilo/Bot-Of-Legends/master/version/MorganaGod.version"
        ToUpdate.ScriptPath = "raw.githubusercontent.com/Artrilo/Bot-Of-Legends/master/MorganaGod.lua"
...
function OnLoad()
    ...
    Q = _Spell({Slot = _Q, DamageName = "Q", Range = 1175, Width = 80, Delay = 0.25, Speed = 1200, Aoe = false, Collision = true, Type = SPELL_TYPE.LINEAR}):AddDraw()
    W = _Spell({Slot = _W, DamageName = "W", Range = 900, Width = 175, Delay = 0.25, Speed = math.huge, Aoe = true, Collision = false, Type = SPELL_TYPE.CIRCULAR}):AddDraw()
    E = _Spell({Slot = _E, DamageName = "E", Range = 750, Type = SPELL_TYPE.TARGETTED}):AddDraw()
    ...
    Menu:addSubMenu(myHero.charName.." - Combo Settings", "Combo")
        ...
        Menu.Combo:addSubMenu("Q Settings", "Q")
            ...
            for i, enemy in pairs(GetEnemyHeroes()) do
                Menu.Combo.Q:addParam(enemy.charName, "Do Not Q "..enemy.charName, SCRIPT_PARAM_ONOFF, false)
            end
            Menu.Combo.Q:addParam("blkhp", "Q if Target HP", SCRIPT_PARAM_SLICE, 10, 0, 100, 0)
        Menu.Combo:addSubMenu("W Settings", "W")
            Menu.Combo.W:addParam("useW", "Use W", SCRIPT_PARAM_ONOFF, true)
            Menu.Combo.W:addParam("WaitW", "Wait W for Q Hit", SCRIPT_PARAM_ONOFF, true)
    ...
    Menu:addSubMenu(myHero.charName.." - Harass Settings", "Harass")
        ...
        Menu.Harass:addParam("WaitW", "Wait W for Q Hit", SCRIPT_PARAM_ONOFF, true)
    ...
    Menu:addSubMenu(myHero.charName.." - Auto Settings", "Auto")
        ...
        Menu.Auto:addSubMenu("Use E To Fuck Spells", "UseE")
...
function Combo()
    ...
    if ValidTarget(target) then
        if Menu.Combo.W.useW then
            if Menu.Combo.W.WaitW and TargetHaveBuff("DarkBindingMissile", target) then
                W:Cast(target)
            elseif not Menu.Combo.W.WaitW then
                W:Cast(target)
            end
        end
    ...
    if myHero.mana / myHero.maxMana * 100 >= Menu.Harass.Mana then
        if ValidTarget(target) then
            ...
            if Menu.Harass.useW then
                if Menu.Harass.WaitW and TargetHaveBuff("DarkBindingMissile", target) then
                    W:Cast(target)
                elseif not Menu.Harass.WaitW then
                    W:Cast(target)
                end
            end
        end
