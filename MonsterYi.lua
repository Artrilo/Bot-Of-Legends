...
function OnLoad()
    ...
    Q = _Spell({Slot = _Q, DamageName = "Q", Range = 600, Type = SPELL_TYPE.TARGETTED}):AddDraw()
    W = _Spell({Slot = _W, DamageName = "W", Range = 0, Type = SPELL_TYPE.SELF}):AddDraw()
    E = _Spell({Slot = _E, DamageName = "E", Range = 189, Type = SPELL_TYPE.SELF}):AddDraw()
    Ignite = _Spell({Slot = FindSummonerSlot("summonerdot"), DamageName = "IGNITE", Range = 600, Type = SPELL_TYPE.TARGETTED})
    R = _Spell({Slot = _R, DamageName = "R", Range = 0, Type = SPELL_TYPE.SELF}):AddDraw()
    ...
    Menu:addSubMenu(myHero.charName.." - Misc Settings", "Misc")
        Menu.Misc:addParam("SetSkin", "Select Skin", SCRIPT_PARAM_LIST, 7, {"1", "2", "3", "4", "5", "6", "7"})
