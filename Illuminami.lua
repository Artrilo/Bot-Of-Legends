...
function CheckUpdate()
    if AUTOUPDATES then
        ...
        ToUpdate.VersionPath = "raw.githubusercontent.com/Artrilo/Bot-Of-Legends/master/version/Illuminami.version"
        ToUpdate.ScriptPath = "raw.githubusercontent.com/Artrilo/Bot-Of-Legends/master/Illuminami.lua"
...
function OnLoad()
    ...
    Q = _Spell({Slot = _Q, DamageName = "Q", Range = 875, Width = 200, Delay = 1.0, Speed = 1750, Aoe = true, Collision = false, Type = SPELL_TYPE.CIRCULAR}):AddDraw()
    W = _Spell({Slot = _W, DamageName = "W", Range = 725, Type = SPELL_TYPE.TARGETTED}):AddDraw()
    E = _Spell({Slot = _E, DamageName = "E", Range = 800, Type = SPELL_TYPE.TARGETTED}):AddDraw()
    Ignite = _Spell({Slot = FindSummonerSlot("summonerdot"), DamageName = "IGNITE", Range = 600, Type = SPELL_TYPE.TARGETTED})
    R = _Spell({Slot = _R, DamageName = "R", Range = 2750, Width = 250, Delay = 0.5, Speed = 850, Aoe = true, Collision = false, Type = SPELL_TYPE.LINEAR}):AddDraw()
    ...
    Menu:addSubMenu(myHero.charName.." - Combo Settings", "Combo")
        ...
        Menu.Combo:addSubMenu("W Settings", "W")
            ...
            Menu.Combo.W:addParam("HealSelf", "Heal Yourself", SCRIPT_PARAM_ONOFF, true)
            Menu.Combo.W:addParam("HP", "% To Heal", SCRIPT_PARAM_SLICE, 70, 0, 100, 0)
            Menu.Combo.W:addParam("HealAlly", "Auto Heal Allies", SCRIPT_PARAM_ONOFF, false)
            Menu.Combo.W:addParam("AllyHP", "% To Heal", SCRIPT_PARAM_SLICE, 50, 0, 100, 0)
        Menu.Combo:addSubMenu("E Settings", "E")
            Menu.Combo.E:addParam("useE", "Use E on Self", SCRIPT_PARAM_ONOFF, true)
            Menu.Combo.E:addParam("useE2", "Use E on Ally", SCRIPT_PARAM_ONOFF, true)
    ...
    Menu:addSubMenu(myHero.charName.." - Misc Settings", "Misc")
        Menu.Misc:addParam("SetSkin", "Select Skin", SCRIPT_PARAM_LIST, 6, {"1", "2", "3", "4", "5", "6", "7"})
...
function OnTick()
    ...
    if Menu.Combo.W.HealSelf then
        HealSelf()
    end
    if Menu.Combo.W.HealAlly then
        HealAlly()
    end
...
-- Health Shit
function HealSelf()
    if Menu.Combo.W.HealSelf and myHero.health/myHero.maxHealth * 100 <= Menu.Combo.W.HP then
        CastSpell(_W, myHero)
    end
end
...
function HealAlly()
    for i, ally in ipairs(GetAllyHeroes()) do
        if ally ~= nil and GetDistance(ally) < W.Range and not ally.dead and W:IsReady() and Menu.Combo.W.HealAlly and ally.health / ally.maxHealth * 100 <= Menu.Combo.W.AllyHP then
            CastSpell(_W, ally)
        end
    end
end
